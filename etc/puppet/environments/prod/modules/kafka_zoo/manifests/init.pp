class kafka_zoo($apps_dir, $zookeepr_dir, $tick_time, $init_limit, $sync_limit, $zoo_data_dir, $servers, $zoo_port, $kafka_ver, $kafka_port, $zoo_minmem, $zoo_maxmem, $zoo_myid_nodeno, $kafka_dir, $zoo_log_dir, $zookeeper_user, $zookeeper_group, $broker_id, $kafka_user, $kafka_group,  $kafka_logs_dir, $kafka_partitions, $kafka_metric_dir, $consumer_servers, $kafka_consumer_grp_id, $producers_brker_list, $kafka_syslog_dir, $kafka_syslog_file, $kafka_num_network_threads, $kafka_num_io_threads, $kafka_socket_send_buffer_bytes, $kafka_socket_receive_buffer_bytes, $kafka_socket_request_max_bytes,$kafka_log_flush_interval_message, $kafka_log_flush_interval_ms, $kafka_log_segment_bytes, $kafka_log_cleanup_interval_mins, $kafka_zookeeper_conn_timeout_ms, $kafka_producer_type, $kafka_compression_codec, $kafka_serializer_class){
    ...
    file {"$kafka_dir":
        ...
        owner   => 'appsuser',
        group   => 'appsuser',
        source => "puppet:///modules/common_files/package/kafka_${kafka_ver}/",
        require => Exec["mkdir_p_${zookeepr_dir}"]
    }
    ...
    file { "${kafka_dir}/bin":
        ...
        mode    => "0755",
        recurse => "true",
        source  => "puppet:///modules/common_files/package/kafka_${kafka_ver}/bin",
        require => File["${kafka_dir}"],
    }
